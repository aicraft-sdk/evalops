...
export default function Analytics() {
  ...
  const { data: trendsData, isLoading: trendsLoading, error: trendsError } = useQuery({
    ...
    queryFn: () => fetch(`/api/analytics/trends?days=${timeRange}`).then(res => res.json()),
    retry: false
  });

  const { data: costData, isLoading: costLoading } = useQuery({
    queryKey: ["/api/analytics/cost-breakdown", timeRange],
    queryFn: () => fetch(`/api/analytics/cost-breakdown?days=${timeRange}`).then(res => res.json()),
    retry: false
  });

  const { data: performanceData, isLoading: performanceLoading } = useQuery({
    queryKey: ["/api/analytics/performance-comparison", timeRange],
    queryFn: () => fetch(`/api/analytics/performance-comparison?days=${timeRange}`).then(res => res.json()),
    retry: false
  });
