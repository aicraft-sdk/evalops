...
For prose accents, see "Prose Page Elements" in `.cursor/skills/documentation/visual-explainer/references/css-patterns.md`. For everything else, use the standard freeform approach with aesthetic directions below.
...
- For text-heavy architecture overviews (card content matters more than topology): read `.cursor/skills/documentation/visual-explainer/templates/architecture.html`
- For flowcharts, sequence diagrams, ER, state machines, mind maps: read `.cursor/skills/documentation/visual-explainer/templates/mermaid-flowchart.html`
- For data tables, comparisons, audits, feature matrices: read `.cursor/skills/documentation/visual-explainer/templates/data-table.html`
- For slide deck presentations (when `--slides` flag is present or `/generate-slides` is invoked): read `.cursor/skills/documentation/visual-explainer/templates/slide-deck.html` and `.cursor/skills/documentation/visual-explainer/references/slide-patterns.md`
- For prose-heavy publishable pages (READMEs, articles, blog posts, essays): read the "Prose Page Elements" section in `.cursor/skills/documentation/visual-explainer/references/css-patterns.md` and "Typography by Content Voice" in `.cursor/skills/documentation/visual-explainer/references/libraries.md`

**For CSS/layout patterns and SVG connectors**, read `.cursor/skills/documentation/visual-explainer/references/css-patterns.md`.

**For pages with 4+ sections** (reviews, recaps, dashboards), also read `.cursor/skills/documentation/visual-explainer/references/responsive-nav.md` for section navigation with sticky sidebar TOC on desktop and horizontal scrollable bar on mobile.
...
**Mermaid theming:** Always use `theme: 'base'` with custom `themeVariables` so colors match your page palette. Use `layout: 'elk'` for complex graphs (requires the `@mermaid-js/layout-elk` package — see `.cursor/skills/documentation/visual-explainer/references/libraries.md` for the CDN import). Override Mermaid's SVG classes with CSS for pixel-perfect control. See `.cursor/skills/documentation/visual-explainer/references/libraries.md` for full theming guide.
...
**Mermaid scaling:** Complex diagrams with 10+ nodes render too small by default. Increase `fontSize` in themeVariables to 18-20px (default is 16px), or apply a CSS `transform: scale(1.3)` on the SVG. Don't let diagrams float in oversized containers with unreadable text. See `.cursor/skills/documentation/visual-explainer/references/css-patterns.md` "Scaling Small Diagrams".

**Mermaid layout direction:** Prefer `flowchart TD` (top-down) over `flowchart LR` (left-to-right) for complex diagrams. LR spreads horizontally and makes labels unreadable when there are many nodes. Use LR only for simple 3-4 node linear flows. See `.cursor/skills/documentation/visual-explainer/references/libraries.md` "Layout Direction: TD vs LR".
...
See `.cursor/skills/documentation/visual-explainer/references/css-patterns.md` for image container styles (hero banners, inline illustrations, captions).
...
**Typography is the diagram.** Pick a distinctive font pairing from the list in `.cursor/skills/documentation/visual-explainer/references/libraries.md`. Every page should use a different pairing from recent generations.
...
Use a real `<table>` element — not CSS Grid pretending to be a table. Tables get accessibility, copy-paste behavior, and column alignment for free. The reference template at `.cursor/skills/documentation/visual-explainer/templates/data-table.html` demonstrates all patterns below.
...
**Code blocks require explicit formatting.** Without `white-space: pre-wrap`, code runs together into an unreadable wall. See the "Code Blocks" section in `.cursor/skills/documentation/visual-explainer/references/css-patterns.md` for the correct pattern.
...
Use these sparingly within visual pages to highlight key points or provide breathing room. See "Prose Page Elements" in `.cursor/skills/documentation/visual-explainer/references/css-patterns.md` for CSS patterns.
...
**Before generating slides**, read `.cursor/skills/documentation/visual-explainer/references/slide-patterns.md` (engine CSS, slide types, transitions, nav chrome, presets) and `.cursor/skills/documentation/visual-explainer/templates/slide-deck.html` (reference template showing all 10 types). Also read `.cursor/skills/documentation/visual-explainer/references/css-patterns.md` for shared patterns and `.cursor/skills/documentation/visual-explainer/references/libraries.md` for Mermaid/Chart.js theming.
...
- **No overflow**: Resize the browser to different widths. No content should clip or escape its container. Every grid and flex child needs `min-width: 0`. Side-by-side panels need `overflow-wrap: break-word`. Never use `display: flex` on `<li>` for marker characters — it creates anonymous flex items that can't shrink, causing lines with many inline `<code>` badges to overflow. Use absolute positioning for markers instead. See the Overflow Protection section in `.cursor/skills/documentation/visual-explainer/references/css-patterns.md`.
- **Mermaid zoom controls**: Every `.mermaid-wrap` container must have zoom controls (+/−/reset buttons), Ctrl/Cmd+scroll zoom, and click-and-drag panning. Complex diagrams render too small without them. The cursor should change to `grab` when zoomed in and `grabbing` while dragging. See `.cursor/skills/documentation/visual-explainer/references/css-patterns.md` for the full pattern.
...
**Required:** Pick from the font pairings in `.cursor/skills/documentation/visual-explainer/references/libraries.md`. Every generation should use a different pairing from the last.
