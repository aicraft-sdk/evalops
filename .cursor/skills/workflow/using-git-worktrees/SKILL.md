...
When running multiple Cursor instances with worktrees, organize with a "cascade" pattern:
...
**Benefits**:
- Clear organization of parallel work
- Prevents context switching overload
- Maintains focus on active tasks
...
For new projects or major features, start with 2 Cursor instances:
...
**Workflow**:
1. Create two worktrees: `scaffold` and `research`
...
**Key Principles**:
- **Well-Defined Scope**: Each instance has clear, non-overlapping scope
- **Minimal Overlap**: Avoid code changes that overlap between instances
- **Use Worktrees**: Essential for parallel instances working on code
- **Name Conversations**: Use `/rename` to name all chats for clarity
...
**Best Practices**:
1. **Define Scope Clearly**: Each instance knows exactly what it's working on
2. **Use Worktrees**: Isolated workspaces prevent conflicts
3. **Name Everything**: Use `/rename` for clarity
4. **Monitor Overlap**: Watch for overlapping code changes
5. **Focus on 3-4 Tasks**: Don't spread too thin
...
- **subagent-orchestration** - For managing parallel agent workflows
