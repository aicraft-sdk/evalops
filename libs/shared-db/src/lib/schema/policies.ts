...
export const policyViolationRelations = relations(
  policyViolations,
  ({ one }) => ({
    organization: one(organizations, {
...
export const insertPolicySchema = (createInsertSchema(policies) as any).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
} as const);

export const insertPolicyViolationSchema = (
  createInsertSchema(policyViolations) as any
