...
export const organizationProviderConfigs = pgTable(
...
export const modelComparisons = pgTable('model_comparisons', {
  id: varchar('id')
    ...
    .default(sql`gen_random_uuid()`),
  organizationId: varchar('organization_id').notNull(),
  baseModelId: varchar('base_model_id')
    .notNull()
    .references(() => models.id),
  compareModelId: varchar('compare_model_id')
    .notNull()
    .references(() => models.id),
...
export const modelBenchmarksRelations = relations(
  modelBenchmarks,
  ({ one }) => ({
    model: one(models, {
...
export const organizationProviderConfigsRelations = relations(
...
export const providerHealthChecksRelations = relations(
...
// Insert schemas for AI Provider tables - using type assertion to fix type errors
export const insertAiProviderSchema = (
  createInsertSchema(aiProviders) as any
).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
...
export const insertModelSchema = (createInsertSchema(models) as any).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertOrganizationProviderConfigSchema = (
  createInsertSchema(organizationProviderConfigs) as any
).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
...
export const insertModelUsageSchema = (
  createInsertSchema(modelUsage) as any
).omit({
  id: true,
  createdAt: true,
});

export const insertProviderHealthCheckSchema = (
  createInsertSchema(providerHealthChecks) as any
).omit({
  id: true,
  checkedAt: true,
});
...
export const insertModelVersionSchema = (
  createInsertSchema(modelVersions) as any
).omit({
  id: true,
  createdAt: true,
});

export const insertModelBenchmarkSchema = (
  createInsertSchema(modelBenchmarks) as any
).omit({
  id: true,
  createdAt: true,
});

export const insertModelComparisonSchema = (
  createInsertSchema(modelComparisons) as any
).omit({
  id: true,
  createdAt: true,
});
...
export type OrganizationProviderConfig =
  typeof organizationProviderConfigs.$inferSelect;
export type InsertOrganizationProviderConfig = z.infer<
  typeof insertOrganizationProviderConfigSchema
>;
...
export type InsertProviderHealthCheck = z.infer<
  typeof insertProviderHealthCheckSchema
>;
