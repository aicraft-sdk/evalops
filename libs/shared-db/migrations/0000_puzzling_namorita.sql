...
ALTER TABLE "evaluator_versions" ADD CONSTRAINT "evaluator_versions_previous_version_id_evaluator_versions_id_fk" FOREIGN KEY ("previous_version_id") REFERENCES "public"."evaluator_versions"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "agents_org_active_idx" ON "agents" USING btree ("organization_id","active");